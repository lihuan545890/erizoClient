...
#include "webrtc/modules/video_capture/mac/avf/device_info_avf.h"
...
static NSArray *camera_presets = @[AVCaptureSessionPreset320x240,
...
#define AVF_UNSUPPORTED()                                  \
  WEBRTC_TRACE(kTraceError,                                \
               kTraceVideoCapture,                         \
               _id,                                        \
               "%s is not supported on the Mac platform.", \
               __FUNCTION__);                              \
  return -1;

DeviceInfoAvf::DeviceInfoAvf(const int32_t device_id)
    : DeviceInfoImpl(device_id) {
  this->Init();
}

DeviceInfoAvf::~DeviceInfoAvf() {}

int32_t DeviceInfoAvf::Init() {
  // Fill in all device capabilities.

  int deviceCount = [DeviceInfoAvfObjC captureDeviceCount];

  for (int i = 0; i < deviceCount; i++) {
    AVCaptureDevice *avDevice = [DeviceInfoAvfObjC captureDeviceForIndex:i];
    ...
    for (NSString *preset in camera_presets) {
      ...
      if (support) {
        VideoCaptureCapability capability =
            [DeviceInfoAvfObjC capabilityForPreset:preset];
...
uint32_t DeviceInfoAvf::NumberOfDevices() {
  return [DeviceInfoAvfObjC captureDeviceCount];
}

int32_t DeviceInfoAvf::GetDeviceName(uint32_t deviceNumber,
                                     char* deviceNameUTF8,
  ...
  NSString* deviceName = [DeviceInfoAvfObjC deviceNameForIndex:deviceNumber];

  NSString* deviceUniqueId =
      [DeviceInfoAvfObjC deviceUniqueIdForIndex:deviceNumber];
...
int32_t DeviceInfoAvf::NumberOfCapabilities(const char* deviceUniqueIdUTF8) {
...
int32_t DeviceInfoAvf::GetCapability(const char* deviceUniqueIdUTF8,
...
int32_t DeviceInfoAvf::DisplayCaptureSettingsDialogBox(
...
int32_t DeviceInfoAvf::GetOrientation(const char* deviceUniqueIdUTF8,
...
int32_t DeviceInfoAvf::CreateCapabilityMap(const char* deviceUniqueIdUTF8) {
