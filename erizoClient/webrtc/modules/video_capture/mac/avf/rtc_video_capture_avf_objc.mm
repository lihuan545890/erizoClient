...
#import "webrtc/modules/video_capture/mac/avf/device_info_avf_objc.h"
#import "webrtc/modules/video_capture/mac/avf/rtc_video_capture_avf_objc.h"
#include "webrtc/system_wrappers/include/trace.h"

using namespace webrtc;
using namespace webrtc::videocapturemodule;

@interface RTCVideoCaptureAvfObjC (hidden)
- (int)changeCaptureInputWithName:(NSString*)captureDeviceName;
@end

@implementation RTCVideoCaptureAvfObjC {
  webrtc::videocapturemodule::VideoCaptureAvf* _owner;
...
- (id)initWithOwner:(VideoCaptureAvf*)owner captureId:(int)captureId {
  if (self == [super init]) {
    _owner = owner;
  ...
  if ([_captureSession
    ...
    if (capability.width > 1280 || capability.height > 720) {
      return NO;
                  ...
                  canSetSessionPreset:AVCaptureSessionPreset960x540]) {
    if (capability.width > 960 || capability.height >540){
      return NO;
    }
  } else if ([_captureSession
    ...
    if (capability.width > 352 || capability.height > 288) {
      return NO;
                  ...
                  canSetSessionPreset:AVCaptureSessionPreset320x240]) {
...
- (void)startCaptureInBackgroundWithOutput:
  ...
  } else if (_capability.width >= 960 || _capability.height >= 540) {
    captureQuality = [NSString stringWithString:AVCaptureSessionPreset960x540];
  ...
  } else if (_capability.width >= 320 || _capability.width >= 240) {
    captureQuality = [NSString stringWithString:AVCaptureSessionPreset320x240];
  }
...
- (BOOL)changeCaptureInputByUniqueId:(NSString*)uniqueId {
  ...
  int captureDeviceCount = [DeviceInfoAvfObjC captureDeviceCount];
  if (captureDeviceCount <= 0) {
    return NO;
  }

  AVCaptureDevice* captureDevice =
      [DeviceInfoAvfObjC captureDeviceForUniqueId:uniqueId];
